...
public class EventCentreActivity extends AppCompatActivity implements SearchView.OnQueryTextListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(getResources().getBoolean(R.bool.portrait_only)){
            ...
            // locking out portrait screen orientation for tablets
        } if(getResources().getBoolean(R.bool.landscape_only)) {
            setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
        }
        setContentView(R.layout.activity_event_centre);
        ...
        agentPostRecyclerAdapter = new AgentPostRecyclerAdapter(post_list, EventCentreActivity.this);
        ...
        firebaseFirestore.collection("Posts").whereEqualTo("category", "Event Centres").limit(5).addSnapshotListener(new EventListener<QuerySnapshot>() {
            @Override
        ...
        Query mQuery = firebaseFirestore.collection("Posts").whereEqualTo("category", "Event Centres");
    ...
    private void loadMorePosts() {
        firebaseFirestore.collection("Posts").whereEqualTo("category", "Event Centres").startAfter(lastVisible).limit(5).addSnapshotListener(new EventListener<QuerySnapshot>() {
            @Override
            public void onEvent(QuerySnapshot queryDocumentSnapshots, FirebaseFirestoreException e) {
                if (!queryDocumentSnapshots.isEmpty()) {
                    lastVisible = queryDocumentSnapshots.getDocuments().get(queryDocumentSnapshots.size() -1);
    ...
    public void onBackPressed() {
        Intent intent = new Intent(EventCentreActivity.this, MainActivity.class);
        startActivity(intent);
        finish();
    ...
    public boolean onQueryTextChange(String newText) {
        List<AgentPost> filteredList = filter(post_list, newText);
        agentPostRecyclerAdapter.updateList(filteredList);
        return true;
    }
